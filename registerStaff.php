<?php 
	...
	{
		$staffID = mysqli_real_escape_string($checkConnect, $_POST['staffID']);
		...
		$insert = "INSERT INTO staff (staffID, name, icNo, address1, address2, postcode, city, state, phoneNo, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$statement = mysqli_prepare($checkConnect, $insert);
		mysqli_stmt_bind_param($statement, "ssssssssss", $staffID, $name, $icNo, $address1, $address2, $postCode, $city, $state, $phoneNo, $getEmail);
		mysqli_stmt_execute($statement);
		mysqli_stmt_close($statement);
		$getData = "SELECT id FROM staff WHERE name='$name'";
		$result = mysqli_query($checkConnect, $getData) or die(mysql_error());
		$result2 = mysqli_fetch_assoc($result);
		$getID = $result2['id'];
		$insert1 = "UPDATE login SET staff_id='$getID' WHERE username='$getUsername'";
...
<div class="container">
    <div class="card card1-container">
		<strong style="font-size: 18px">Staff Details</strong></br>List of Staff Details.</br>
			<form role="form" data-toggle="validator" onSubmit="return validation()" id="registration" class="form-signin" method="post" action="registerStaff.php">
				...
				<div class="icon-addon addon-lg">
					<input type="text" id="staffID" name="staffID" pattern="[0-9]+" title="Must be Numeric Only!" maxlength="10" class="form-control" placeholder="Staff ID" required autofocus>
						<label for="staffID" class="glyphicon glyphicon-tag" rel="tooltip" title="staffID"></label>
					</input>
				</div>
				<div class="icon-addon addon-lg">
