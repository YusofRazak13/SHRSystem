<?php 
	...
	if(isset($_POST['btn-register']))
	{
		...
		$insert = "INSERT INTO houseowner (name, icNo, address1, address2, postcode, city, state, phoneNo, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$statement = mysqli_prepare($checkConnect, $insert);
		mysqli_stmt_bind_param($statement, "sssssssss", $name, $icNo, $address1, $address2, $postCode, $city, $state, $phoneNo, $getEmail);
		mysqli_stmt_execute($statement);
		mysqli_stmt_close($statement);
		$getData = "SELECT id FROM houseowner WHERE name='$name'";
		$result = mysqli_query($checkConnect, $getData) or die(mysql_error());
		$result2 = mysqli_fetch_assoc($result);
		$getID = $result2['id'];
		$insert1 = "UPDATE login SET houseOwner_id='$getID' WHERE username='$getUsername'";
...
<div class="container">
    <div class="card card1-container">
		<strong style="font-size: 18px">House Owner Details</strong></br>List of House Owner Details.</br>
			<form role="form" data-toggle="validator" onSubmit="return validation()" id="registration" class="form-signin" method="post" action="registerHouseOwner.php">
				...
				<div class="icon-addon addon-lg">
					<input type="text" id="name" name="name" pattern="[ a-zA-Z/]+" title="Must be Alphabet Only!" class="form-control" placeholder="Full Name" required autofocus>
